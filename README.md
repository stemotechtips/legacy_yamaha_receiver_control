...
The interface itself is a basic HTML page with Javascript, but all of the communication with the device is done with HTTP post requests.  The Javascript polls each of the zones for the receiver every second or so to check for the status, and then also delivers any user commands through the same means.
...
This library is a basic asynchronous library that uses the Python 'aiohttp' library to issue post requests to the receiver. It is anticipated that this will be used with a Home Assistant integration that then uses this library for communication with the receiver.
...
This is not a unique experience for this receiver.  Much of the equipment from this era has functions that are locked behind dated platforms, apps that no longer exist, or long-forgotten protocols.  This is a basic library that reinstates that functionality, helping to bringing this (very capable) hardware into the modern era. After all, this receiver can do 1080p, 7.1 surround sound, 3 zone audio and a huge variety of analog and digital inputs - it's no slouch!
