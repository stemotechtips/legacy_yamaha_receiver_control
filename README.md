...
Running `yamaha-receiver` without arguments prompts for the receiver IP address and then accepts commands until `quit` is entered. You can also provide the IP address immediately with `yamaha-receiver 192.168.1.XX`; this starts interactive mode without prompting for the address. 
